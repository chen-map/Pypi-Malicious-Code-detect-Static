...
class TestDataSynthesizer:
    ...
    RESERVED_NAMES = {
        ...
        # 函数名不能是关键字
        'return', 'lambda', 'class', 'def'  # 添加更多危险的关键字
    }
    ...
    def synthesize(self, analysis: Dict, source_code: str = None) -> Dict[str, str]:
        ...
        if source_code:
            ...
            # 改进的正则匹配：支持更复杂的字符串情况
            # 1. 匹配双引号字符串（可能包含单引号、转义字符）
            pattern_double = r'(\w+)\s*=\s*"((?:[^"\\]|\\.){10,})"'
            # 2. 匹配单引号字符串（可能包含双引号、转义字符）
            pattern_single = r"(\w+)\s*=\s*'((?:[^'\\]|\\.){10,})'"

            matches = []
            matches.extend(re.findall(pattern_double, source_code))
            matches.extend(re.findall(pattern_single, source_code))
            ...
            for var_name, string_value in matches:
                ...
                # 额外过滤：排除明显的自然语言句子（包含常见英语单词）
                # 检查是否包含常见的短单词（可能是自然语言）
                if re.search(r'\b(the|and|is|at|which|on)\b', string_value, re.IGNORECASE):
                    # 但如果是URL/路径/命令，仍然保留
                    if not any(prefix in string_value for prefix in ['http', 'https', 'ftp', '://', '/', '\\', '.', 'cmd', 'sh']):
                        continue

                # 保留所有可疑的长字符串（包括加密的URL、混合引号的URL等）
                if var_name not in test_inputs:
                    # 清理转义字符，但保留必要的转义
                    clean_value = string_value.replace('\\', '\\\\')
                    test_inputs[var_name] = f'"{clean_value}"'
...
'''
    ...
    def _generate_function_call(self, call_node: Dict, module_name: str, test_inputs: Dict) -> List[str]:
        ...
        if not is_builtin:
            lines.append(f"\n        # 测试模块: {module_name}")
            lines.append(f"        # 策略: 优先尝试import，失败则使用importlib加载.py文件")
            lines.append(f"        try:")
            lines.append(f"            import {module_name}")
            lines.append(f"        except ImportError as e1:")
            lines.append(f"            print(f'[WARN] Normal import failed: {{e1}}')")
            lines.append(f"            print(f'[INFO] Trying to load .py file as module using importlib...')")
            lines.append(f"            try:")
            lines.append(f"                import importlib.util")
            lines.append(f"                import sys")
            lines.append(f"                from pathlib import Path")
            lines.append(f"                ")
            lines.append(f"                # 查找.py文件")
            lines.append(f"                py_file = package_dir / '{module_name}.py'")
            lines.append(f"                if not py_file.exists():")
            lines.append(f"                    # 尝试子目录")
            lines.append(f"                    py_file = None")
            lines.append(f"                    for subdir in package_dir.iterdir():")
            lines.append(f"                        if subdir.is_dir():")
            lines.append(f"                            candidate = subdir / '{module_name}.py'")
            lines.append(f"                            if candidate.exists():")
            lines.append(f"                                py_file = candidate")
            lines.append(f"                                break")
            lines.append(f"                ")
            lines.append(f"                if py_file and py_file.exists():")
            lines.append(f"                    print(f'[INFO] Found .py file: {{py_file}}')")
            lines.append(f"                    # 使用importlib加载.py文件为模块")
            lines.append(f"                    spec = importlib.util.spec_from_file_location('{module_name}', py_file)")
            lines.append(f"                    if spec and spec.loader:")
            lines.append(f"                        {module_name}_module = importlib.util.module_from_spec(spec)")
            lines.append(f"                        sys.modules['{module_name}'] = {module_name}_module")
            lines.append(f"                        spec.loader.exec_module({module_name}_module)")
            lines.append(f"                        print(f'[OK] Successfully loaded {{py_file}} as module')")
            lines.append(f"                    else:")
            lines.append(f"                        raise ImportError('Failed to create module spec')")
            lines.append(f"                else:")
            lines.append(f"                    raise ImportError(f'Cannot find .py file for module {module_name}')")
            lines.append(f"            except Exception as e2:")
            lines.append(f"                print(f'[ERROR] Failed to load .py file: {{e2}}')")
        ...
        # 检查函数名是否是Python关键字（如 return, lambda, class等）
        if func_name in TestDataSynthesizer.RESERVED_NAMES:
            lines.append(f"\n        # 函数名: {func_name}")
            lines.append(f"        # 源模块: {module_name}")
            lines.append(f"        # [WARNING] 函数名是Python关键字，无法直接调用")
            lines.append(f"        # 说明: '{func_name}' 是Python保留关键字，原代码使用了属性访问")
            lines.append(f"        # 原始调用: {module_name}.{func_name}({call_args})")
            lines.append(f"        # 跳过此调用（语法错误）")
            return lines
        ...
        # 避免嵌套f-string问题：先转义特殊字符
        safe_call_display = call_code.replace('"', '\\"').replace("'", "\\'")
        lines.append(f'\n        print(f"[CALL] Executing: {repr("{safe_call_display}")}")')
        lines.append(f"        try:")
    ...
    def _generate_direct_execution(self, node: Dict, test_inputs: Dict) -> List[str]:
        ...
        lines.append(f"            except Exception as e2:")
        lines.append(f"                print(f'[ERROR] Failed to load .py file: {{e2}}')")
        lines.append(f"            # 不return，继续执行代码（可能是独立代码，不需要模块）")
        ...
        # 特殊处理：函数定义、类定义等声明语句不能作为表达式执行
        if (exec_code.strip().startswith('def ') or
            exec_code.strip().startswith('class ') or
            exec_code.strip().startswith('async def ') or
            exec_code.strip().startswith('@')):  # 装饰器
            lines.append(f"        # 函数/类定义 - 声明语句，无法直接执行")
            lines.append(f"        # 原始代码: {exec_code[:80]}")
            lines.append(f"        # 说明: 这是函数定义，需要在调用时才能执行")
            return lines

        # 检查是否为多行代码
        if '\n' in exec_code or ';' in exec_code:
            # 多行代码：使用exec()执行
            # 🔥 Bug修复：提取exec代码中使用的模块，并添加import语句
            required_modules = self._extract_modules_from_code(exec_code)
            if required_modules:
                lines.append(f"        # 导入exec代码中使用的模块")
                for mod in required_modules:
                    lines.append(f"        import {mod}")
                lines.append(f"")
            # 使用更安全的方式：转义特殊字符，避免三引号冲突
            safe_exec_code = exec_code.replace('\\', '\\\\').replace('"', '\\"').replace('\n', '\\n')
            lines.append(f"        print(f'[EXEC] Executing multi-line code...')")
            lines.append(f"        try:")
            lines.append(f'            exec("{safe_exec_code}")')
            ...
            # 单行代码：直接执行
            # 🔥 Bug修复：提取单行代码中使用的模块，并添加import语句
            required_modules = self._extract_modules_from_code(exec_code)
            if required_modules:
                lines.append(f"        # 导入单行代码中使用的模块")
                for mod in required_modules:
                    lines.append(f"        import {mod}")
                lines.append(f"")
            # 先计算安全的代码片段（用于打印），避免复杂的嵌套f-string
            safe_code_snippet = exec_code[:80].replace('\\', '\\\\').replace('"', '\\"').replace("'", "\\'")
            lines.append(f"        print(f'[EXEC] Executing: {{repr(\"{safe_code_snippet}\")}}...')")
            lines.append(f"        try:")
    ...
    def _extract_modules_from_code(self, code: str) -> List[str]:
        """
        从代码中提取使用的模块名（用于exec代码）
        ...
        # 常见的Python标准库模块（这些模块如果出现在代码中，需要导入）
        common_modules = {
            'os', 'sys', 'subprocess', 'json', 're', 'pathlib', 'shutil',
            'urllib', 'requests', 'http', 'ftplib', 'socket', 'ssl',
            'pickle', 'marshal', 'base64', 'hashlib', 'uuid',
            'datetime', 'time', 'random', 'math', 'collections',
            'itertools', 'functools', 'typing', 'io', 'tempfile',
            'threading', 'multiprocessing', 'concurrent'
        }

        # 正则模式：匹配模块.方法() 或 模块.属性 的调用
        # 例如：subprocess.Popen(), os.path.exists(), urllib.request.urlopen()
        pattern = r'\b([a-z_][a-z0-9_]*)\s*\.\s*(?:[a-z_][a-z0-9_]*\s*\(|[A-Z_][A-Z0-9_]*)'
        ...
        # 特殊处理：subprocess非常常见，优先检测
        if 'subprocess' in code.lower() and 'Popen' in code:
            modules.add('subprocess')

        if 'os' in code.lower() and ('path' in code or 'environ' in code or 'system' in code):
            modules.add('os')
